        ...
        public function get_services_by_type($request){
			...
			if( $query->have_posts() ){
				...
				while ($query->have_posts()) : $query->the_post();
					...
					$service_data['employer_title']		= get_the_title($profile_id);
					$service_data['employertagline']	= workreap_get_tagline($profile_id);
					$service_data['employer_avatar'] 	= apply_filters(
					...
					$service_data['employer_verified'] 	= get_post_meta($profile_id, '_is_verified', true);
        ...
        public function get_freelancer_cancelled_jobs($request){
			...
			if( $pquery->have_posts() ){
				...
				global $post;
					...
					$item['status']	    = esc_html__('Cancelled','workreap_api');
			...
			if( $pquery->have_posts() ){
				...
				global $post;
					...
					if( $job_status === 'hired' ) {
						...
						if( !empty($proposal_hiring_status)  && $proposal_hiring_status === 'pending' && $proposal_status === 'on' ) {
