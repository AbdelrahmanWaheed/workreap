				...
				<div class="row justify-content-center">
					...
					<?php if (class_exists('WooCommerce')) { ?>
						<div class="wt-packagestwo">
							<?php if( ( !is_user_logged_in() && $show_tabs === 'yes' ) || ( is_user_logged_in() && $user_type === 'administrator' && $show_tabs === 'yes' ) ) { ?>
								<div class="col-12">
							...
							<?php 
								if ($packages->have_posts()) {
									while ($packages->have_posts()) :
										...
										$package_img	= get_the_post_thumbnail_url($post_id, 'workreap_service_thumnail');
										$role_type		= get_post_meta($post_id, 'package_type', true);

										if (!empty($role_type) && $role_type === 'freelancer') {
											$typeClass	= 'freelancer-packages';
										} else if (!empty($role_type) && $role_type === 'employer' ) {
										...
										<div class="col-md-6 col-lg-4 <?php echo esc_attr($typeClass);?>">
											<div class="wt-packagetwo">
												<div class="wt-package-content">
													...
													<?php }?>
													<strong><sup><?php echo esc_html($currency_symbol['symbol']);?></sup><?php echo esc_html($product->get_price()); ?> <sub>/ <?php echo  esc_html($duration_title);?></sub></strong>
													<?php if (!empty($tax_note)) { ?>
														<em><?php echo esc_html($tax_note); ?> <i class="ti-info-alt wt-tipso tooltip-content tipso_style" data-tipso="<?php echo esc_attr(get_the_title()); ?>"></i></em>
													<?php } ?>
												</div>
												<div class="jb-package-feature">
													...
													<div class="wt-btnarea">
														<a class="wt-btntwo <?php echo esc_attr( $classLogin );?>" data-key="<?php echo intval($post_id);?>" href="javascript:;"><span><?php esc_html_e('Buy Now', 'workreap_core');?></span></a>
													</div>
												</div>
											</div>
										</div>
