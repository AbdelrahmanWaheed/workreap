...
if (!class_exists('Workreap_JobsByLocations')) {
    ...
    class Workreap_JobsByLocations extends WP_Widget {
        ...
        public function widget($args , $instance) {
			...
			$locations_list 	= explode(",", $project_locations);
			$search_page	= '';
        ...
        public function form($instance) {
			...
			$view_all_link  	= !empty($instance['view_all_link']) ? $instance['view_all_link'] : '';
			$project_locations  = !empty($instance['project_locations']) ? explode(",", $instance['project_locations']) : array();
            ...
            <p>
				...
				<?php $args = array(
					...
					foreach( $terms as $id => $name ) { 
						$checked = "";
						if(in_array($name->name, $project_locations)){
							$checked = "checked";
						}
					?>
					<input type="checkbox" class="checkbox" id="<?php echo ( $this->get_field_id('project_locations') ); ?>" name="<?php echo $this->get_field_name('project_locations[]'); ?>" value="<?php echo esc_attr($name->name); ?>"  <?php echo esc_html($checked); ?>/>
        ...
        public function update($new_instance , $old_instance) {
            ...
            $instance['project_locations']  = !empty($new_instance['project_locations']) ? implode(",", $new_instance['project_locations']) : 0;
