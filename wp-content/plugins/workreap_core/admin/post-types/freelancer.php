...
if (!class_exists('Workreap_Freelancer')) {
    ...
    class Workreap_Freelancer {
        ...
        public function prepare_post_type() {
			...
			//for Badges
			$badge_labels = array(
                'name' 				=> esc_html__('Badges', 'workreap_core'),
                'singular_name' 	=> esc_html__('Badges','workreap_core'),
                'search_items' 		=> esc_html__('Search Badge', 'workreap_core'),
                'all_items' 		=> esc_html__('All Badges', 'workreap_core'),
                'parent_item' 		=> esc_html__('Parent Badges', 'workreap_core'),
                'parent_item_colon' => esc_html__('Parent Badges:', 'workreap_core'),
                'edit_item' 		=> esc_html__('Edit Badge', 'workreap_core'),
                'update_item' 		=> esc_html__('Update Badge', 'workreap_core'),
                'add_new_item' 		=> esc_html__('Add New Badge', 'workreap_core'),
                'new_item_name' 	=> esc_html__('New Badge Name', 'workreap_core'),
                'menu_name' 		=> esc_html__('Badges', 'workreap_core'),
            );
		...
		public function balance_history_meta_box_print($post) {
			...
			$current_balance		= workreap_get_sum_earning_freelancer($linked_profile,'completed','freelancer_amount');
			$current_balance		= !empty($current_balance) ? $current_balance : 0 ;
			$available_balance	= workreap_get_sum_earning_freelancer($linked_profile,'hired','freelancer_amount');
			$available_balance	= !empty($available_balance) ? $available_balance : 0 ;
			if(empty( $linked_profile )){return;}
			?>
			<ul class="review-info">
                <li>
                   <span class="push-right">
                    	<?php esc_html_e('Current balance', 'workreap_core'); ?>&nbsp:
                    </span>
		...
		public function linked_profile_meta_box_print($post) {
			...
			<ul class="review-info">
                ...
                <?php if( ( empty( $is_verified ) || $is_verified === 'no' ) && ( isset( $verify_user ) && $verify_user === 'none' )  ){?>
					<li>
						<span class="push-right">
							<a href="javascript:;" class="do_verify_user" data-type="approve" data-id="<?php echo esc_attr( $post->ID );?>"><?php esc_html_e('Approve Account', 'workreap_core'); ?></a>
						</span>
					</li>
                <?php }else{?>
                	<li>
						<span class="push-right">
							<a href="javascript:;" class="do_verify_user" data-type="reject"  data-id="<?php echo esc_attr( $post->ID );?>"><?php esc_html_e('Disable Account', 'workreap_core'); ?></a>
						</span>
					</li>
                <?php }?>
			</ul>
