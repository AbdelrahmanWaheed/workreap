...
if (!class_exists('Workreap_Projects')) {
    ...
    class Workreap_Projects {
        ...
        public function prepare_post_type() {
            ...
            $cat_labels = array(
                'name' 					=> esc_html__('Project Categories', 'workreap_core'),
                'singular_name' 		=> esc_html__('Category','workreap_core'),
                'search_items'			=> esc_html__('Search Category', 'workreap_core'),
                'all_items' 			=> esc_html__('All Category', 'workreap_core'),
                'parent_item' 			=> esc_html__('Parent Category', 'workreap_core'),
                'parent_item_colon' 	=> esc_html__('Parent Category:', 'workreap_core'),
                'edit_item' 			=> esc_html__('Edit Category', 'workreap_core'),
                'update_item' 			=> esc_html__('Update Category', 'workreap_core'),
                'add_new_item' 			=> esc_html__('Add New Category', 'workreap_core'),
                'new_item_name' 		=> esc_html__('New Category Name', 'workreap_core'),
                'menu_name' 			=> esc_html__('Categories', 'workreap_core'),
            );
			...
			if( apply_filters('workreap_system_access','job_base') === true ){
            	register_taxonomy('project_cat', array('projects','micro-services'), $cat_args);
			} else {
				register_taxonomy('project_cat', array('micro-services'), $cat_args);
			}
		...
		public function add_custom_meta_box($post_type,$post) {
			if ($post_type === 'projects') {
				...
				if( ( $post->post_status === 'pending' ) && ( isset( $job_status ) && $job_status === 'pending' )  ){
                ...
                <?php if( ( $post->post_status === 'pending' ) ){?>
					<li>
						<span class="push-right">
							<a href="javascript:;" class="do_approve_post" data-type="project" data-post="<?php echo esc_attr( $post->ID );?>" data-id="<?php echo esc_attr( $linked_profile );?>"><?php esc_html_e('Approve Project', 'workreap_core'); ?></a>
						</span>
					</li>
                <?php }?>
			</ul>
