		...
		public function add_custom_meta_box($post_type,$post) {
			if ($post_type === 'micro-services') {
				...
				if( ( $post->post_status === 'pending' ) && ( isset( $job_status ) && $job_status === 'pending' )  ){
					add_meta_box( 'publish_post', esc_html__('Approve Service', 'workreap_core'), array(&$this, 'approve_service_meta_box_print'), 'micro-services', 'side', 'high');
				}
            }
		}
                ...
                <?php if( ( $post->post_status === 'pending' ) ){?>
					<li>
						<span class="push-right">
							<a href="javascript:;" class="do_approve_post" data-type="service" data-post="<?php echo esc_attr( $post->ID );?>" data-id="<?php echo esc_attr( $linked_profile );?>"><?php esc_html_e('Approve Service', 'workreap_core'); ?></a>
						</span>
					</li>
                <?php }?>
			</ul>
