...
add_image_size('workreap_employer_large', 1110, 300, true); 
...
add_image_size('workreap_employer_search_grid', 350, 172, true);
...
add_image_size('workreap_portfolio_freelancer', 203, 140, true);
add_image_size('workreap_freelancer_thumbnail', 120, 120, true);
add_image_size('workreap_listing_thumb', 100, 100, true);
add_image_size('workreap_service_thumnail', 75, 75, true);
add_image_size('workreap_latest_articles_widget', 65, 65, true); 
add_image_size('workreap_recent_jobs', 50, 50, true); 
...
if (!function_exists('workreap_prepare_pagination')) {
    ...
    function workreap_prepare_pagination($pages = '', $range = 4) {
		...
		if (1 != $pages) {
			 echo "<div class='wt-paginationvtwo'><nav class=\"wt-pagination\"><ul>";

			if ($current_page > 1) {
				echo "<li class='wt-prevpage'><a href='" . esc_url( get_pagenum_link($current_page - 1) ) . "'><i class=\"lnr lnr-chevron-left\"></i></i></a></li>";
			}

			for ($i = 1; $i <= $pages; $i++) {
				if (1 != $pages && (!( $i >= $current_page + $range + 1 || $i <= $current_page - $range - 1 ) )) {
					echo esc_attr( $paged == $i ) ? "<li class=\"wt-active\"><a href='javascript:;'>" . $i . "</a></li>" : "<li><a href='" . esc_url( get_pagenum_link($i) ) . "' class=\"inactive\">" . $i . "</a></li>";
				}
			}

			if ($current_page < $pages) {
				echo "<li class='wt-nextpage'><a href=\"" . esc_url( get_pagenum_link($current_page + 1) ) . "\"><i class=\"lnr lnr-chevron-right\"></i></a></li>";
			}
...
if (!function_exists('workreap_get_post_date')) {
    ...
    function workreap_get_post_date($post_id = '') {
        global $post;
        echo '<time datetime="' . date('Y-m-d', strtotime(get_the_date('Y-m-d', $post_id))) . '"><i class="lnr lnr-clock"></i><span>' . date(get_option('date_format'), strtotime(get_the_date('Y-m-d', $post_id))) . '</span></time>';
    }
...
/**
 * Get CometChat UnRead Messages For Specific User
 */
if (!function_exists('workreap_count_unread_cometchat_msgs')) {
    ...
    function workreap_count_unread_cometchat_msgs($uid = '') {
        $comet_apikey = get_option('cc_api_key');
        if (!empty($comet_apikey) && !empty($uid)) {
            ...
            if(!empty($response) && $response->failed->status != 2001) {
                if (!empty($response) && $response->success->status == 2000) {
