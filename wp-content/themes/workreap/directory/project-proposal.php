	...
	if(!empty($proposal_id) && !empty($post_author) 
	   ...
	   && $allow_proposal_edit == 'yes' 
		...
		if( !empty( $per_hour_amount ) ){
			$proposed_cost	= !empty($proposed_amount) ?( $proposed_amount*$per_hour_amount) : 0.00;
		} else {
			$proposed_cost	= !empty($proposed_amount) ? $proposed_amount : 0.00;
		}
											...
											<ul class="wt-attachfile uploaded-placeholder">
												...
												?>
													<li class="attachment-new-item wt-doc-parent" id="thumb-<?php echo esc_attr($attachment_id);?>">
														<span class="uploadprogressbar" style="width:100%"></span>
														<span><?php echo esc_html($name);?></span>
														<em><?php esc_html_e('File size:', 'workreap'); ?> <?php echo esc_html( size_format($doc_file_size, 2) ); ?><a href="javascript:;" class="lnr lnr-cross wt-remove-attachment"></a></em>
