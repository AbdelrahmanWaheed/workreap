				...
				<div id="wt-twocolumns" class="wt-twocolumns wt-haslayout">
						...
						<aside id="wt-sidebar" class="wt-sidebar">
							...
							<div class="floating-mobile-filter">
								<div class="wt-filter-scroll wt-collapse-filter">
									...
									<form method="get" name="serach-projects" class="search-freelancersform" action="#">
										<h2 class="filter-byhead"><?php esc_html_e('Filter Freelancers By', 'workreap'); ?></h2>
										<?php do_action('workreap_keyword_search'); ?>
										<?php if( apply_filters('workreap_filter_settings','freelancer','type') === 'enable' ){do_action('workreap_print_project_freelancer_type', esc_html__('Freelancer type', 'workreap') );} ?>
										...
										<?php if( apply_filters('workreap_filter_settings','freelancer','english') === 'enable' ){do_action('workreap_print_freelancer_english_level');} ?>
										<?php if( apply_filters('workreap_filter_settings','freelancer','locations') === 'enable' ){do_action('workreap_print_locations');} ?>
										<?php if( apply_filters('workreap_filter_settings','freelancer','languages') === 'enable' ){do_action('workreap_print_languages');} ?>
					...
					<div class="col-xs-12 col-sm-12 col-md-7 col-lg-7 col-xl-8 float-left">
						...
						<ul class="wt-filterholder-three wt-filtertag">
							<?php if( !empty( $sortby ) || !empty( $rating ) ){?>
								<li class="wt-filtertagclear">
									<a href="javascript:;"><i class="fa fa-times"></i> <span><?php esc_html_e('Clear Sorting', 'workreap'); ?></span></a>
								</li>
							<?php }?>
