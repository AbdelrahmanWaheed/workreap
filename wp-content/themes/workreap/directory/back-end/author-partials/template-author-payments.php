				...
				<div class="wt-languagesbox">
					<div class="wt-pkgexpirey sp-pack-note">
						<div class="sp-xs-12 sp-sm-12 sp-md-12 sp-lg-12 pull-left">
							...
							<p><?php esc_html_e('You can also exclude or include number of day from package and featured date.', 'workreap'); ?></p>
						</div>
					</div>
							...
							<div class="wt-timecounter wt-expireytimecounter">
								<div class="package-expireon">
									<?php if ( !empty($package_expiry) && $package_expiry > $today) { ?>
										<p><?php echo date(get_option('date_format'), $package_expiry); ?> <?php esc_html_e('at', 'workreap'); ?> <?php echo date('H:i A', $package_expiry); ?></p>
									<?php } else { ?>
										<?php if ( !empty($package_expiry)) { ?>
											<p><?php esc_html_e('This has expired and expiry date was:', 'workreap'); ?>&nbsp;<strong><?php echo date(get_option('date_format'), $package_expiry); ?> <?php esc_html_e('at', 'workreap'); ?> <?php echo date(get_option('time_format'), $package_expiry); ?></strong></p>
										<?php } ?>
									<?php } ?>
								</div>
								<div class="sp-row">
									<div class="sp-xs-12 sp-sm-12 sp-md-12 sp-lg-12 pull-left">
										<div class="form-group">
											<p><?php esc_html_e('Add number of days to set/update featured expiry date', 'workreap'); ?></p>
											<input type="text" placeholder="<?php esc_attr_e('Add number of days', 'workreap'); ?>" name="package_include" class="form-control" value="" />
										</div>
									</div>
								</div>
								<div class="sp-row">
									<div class="sp-xs-12 sp-sm-12 sp-md-12 sp-lg-12 pull-left">
										<div class="form-group">
											<p><?php esc_html_e('Or remove specific number of days from package expiry date', 'workreap'); ?></p>
											<input type="text" placeholder="<?php esc_attr_e('Exclude number of days', 'workreap'); ?>" name="package_exclude" class="form-control" value="" />
										</div>
									</div>
								</div>
							</div>
