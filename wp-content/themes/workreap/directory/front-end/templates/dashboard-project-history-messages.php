...
$description 		= '';
$name 				= 'chat_desc';	
$settings 			= array('media_buttons' => false,'textarea_name'=> $name,'editor_class'=> 'wt-tinymceeditor','quicktags' => true,'editor_height'=>300 );
$args 				= array(
...
<div class="wt-haslayout wo-project-history">
	<div class="wt-tabscontenttitle">
		<div class="row">
			...
			<div class="col-sm-6">
				<?php $comments_count = get_comments_number( $post_comment_id ); ?>
				<h2 class="float-right"><?php echo esc_html__(sprintf(_n('One Message', '%d Messages', $comments_count), $comments_count), 'workreap'); ?></h2>
			</div>
		</div>
	</div>
	<div class="wt-historycontent">
		...
		<?php if( !empty( $comments ) ){ ?>
			...
			foreach ($comments as $key => $value) { 
				...
				// get new comment flag and clear it
				$new_comment    = get_comment_meta( $comments_ID, '_new_comment', true );
				$new_comment    = $new_comment == true && $user_id != $user_identity ? true : false;
				if( $new_comment == true ) {
					delete_comment_meta( $comments_ID, '_new_comment' );
				}
				...
				?>
				<li class="collapsed <?php echo $new_comment == true ? 'wt-newitem' : ''; ?>" data-toggle="collapse" 
					data-target="#collapse<?php echo esc_attr( $counter ); ?>">
					...
					<div class="wt-rightarea wt-msgbtns">
						<a href="javascript:;" class="wt-btn wt-msgbtn"><i class="lnr lnr-chevron-up"></i><?php esc_html_e('Message', 'workreap'); ?></a>
						<?php if( !empty( $project_files ) ){ ?>
							<a href="javascript:;" data-id="<?php echo esc_attr( $comments_ID ); ?>" class="wt-btn wt-attachmentbtn wt-download-attachment"><i class="lnr lnr-download"></i><?php esc_html_e('Attachment(s)', 'workreap'); ?></a>
						<?php } ?>
					</div>
				</li>
			...
			<form class="wt-formtheme wt-userform wt-project-chat-form">
				<fieldset>
					...
					<div class="form-group wt-btnarea">
						<a href="javascript:;" class="wt-btn wt-submit-project-chat" data-id="<?php echo esc_attr( $post_comment_id ); ?>">
							<?php esc_html_e('Send Now', 'workreap'); ?>
						</a>
					</div>
				</fieldset>
			</form>
		...
		<li class="collapsed" data-toggle="collapse" data-target="#collapse{{data.counter}}">
			...
			<div class="wt-rightarea wt-msgbtns">
				...
				<# if( data.is_files == 'yes' ){ #>
					<a href="javascript:;" class="wt-btn wt-attachmentbtn wt-download-attachment" data-id="{{data.comment_id}}"><i class="lnr lnr-download"></i><?php esc_html_e('Attachment', 'workreap'); ?></a>
				<# } #>
			</div>
		</li>
