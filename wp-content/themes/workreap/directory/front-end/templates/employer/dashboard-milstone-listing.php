												...
												<div class="form-group">
													<span class="wt-select">
														<select id="wt-change-project-status">
															<option value="" disabled=""><?php esc_html_e('Project Status','workreap');?></option>
															<?php 
													...
													<a href="javascript:;" class="wt-searchgbtn" id="btn-change-status"><i class="fa fa-check"></i></a>
												</div>
											</fieldset>
										</form>
									...
									<?php } else{ ?>
										<div class="wt-milestonebtn wt-flexbox">
											<?php if(!empty($post_status) && $post_status === 'cancelled' ){ ?>
												<a href="javascript:;" class="wt-btn" data-toggle="modal" data-target="#wt-projectmodalbox-cancelled"><?php esc_html_e('Cancelled','workreap');?></a>
											<?php } else {
												if(!empty($proposal_status) && $proposal_status === 'pending' ){ ?>
													<a href="javascript:;" class="wt-btn"><?php esc_html_e('Under freelancer review','workreap');?></a>
												<?php } ?>
												<?php if( empty($proposal_status)  && $count_post > 0 && intval($proposal_price) == intval($total_milestone_price)) { ?>
													<a href="javascript:;" class="wt-btn wt-milstone-request" data-id="<?php echo intval($proposal_id);?>"><?php esc_html_e('Send Milestone to Freelancer','workreap');?></a>
												<?php } ?>
												<?php if( $proposal_price > $total_milestone_price ){?>
													<a href="javascript:;" class="wt-btn" data-toggle="modal" data-target="#wt-addmilstone"><?php esc_html_e('Add new milestone','workreap');?></a>
												<?php } ?>
											<?php } ?>
										</div>
									<?php } ?>
											...
											<div class="wt-milestonesingle__active--uppersection">
												...
												<div class="wt-milestonesingle__active--btn">
													<?php if(!empty($updated_status) && $updated_status  ==='pending' ) { ?>
														<a href="javascript:;" data-toggle="modal" data-target="#wt-addmilstone-<?php echo intval($post->ID);?>" class="wt-milestonesingle__active--edit"><?php esc_html_e('Edit','workreap');?></a>
													<?php } ?>
													<a href="javascript:;" data-id="<?php echo intval($post->ID);?>" class="wt-btn <?php echo esc_attr($status_class);?>"><?php echo esc_html(ucfirst($status_text));?></a>
													<?php if($updated_status === 'hired') { ?>
														<a href="javascript:;" data-id="<?php echo intval($post->ID);?>" class="wt-btn wt-milestone-completed wt-bg-green"><?php esc_html_e('Complete Now','workreap');?></a>
													<?php } ?>
													<?php if( !empty($order_id) ){ ?>
														<a href="<?php echo esc_url($order_url);?>" class="wt-btn" target="_blank"><?php esc_html_e('Check Invoice','workreap');?></a>
													<?php } ?>
												</div>
											</div>
										</div>
													...
													<form class="wt-formtheme wt-milestone-form">
														<fieldset>
															...
															<div class="form-group form-group-half wt-inputwithicon" data-provide="datepicker">
																...
																<input id="wt-startdate" type="text" name="due_date" class="form-control <?php echo esc_attr($datepicker_class);?>" placeholder="<?php esc_attr_e('Due Date','workreap');?>" value="<?php echo esc_attr($milstone_date);?>">
															</div>
															<div class="form-group form-group-half">
																<input type="number" name="price" class="form-control" placeholder="<?php esc_attr_e('Milestone Price','workreap');?>" value="<?php echo esc_attr($milstone_price_single);?>" max="<?php echo esc_attr($edit_price);?>">
															</div>
															<div class="form-group">
																<textarea class="form-control" name="description" placeholder="<?php esc_attr_e('Description','workreap');?>"><?php echo do_shortcode($milstone_content);?></textarea>
															</div>
															<div class="form-group wt-btnarea">
																<a href="javascript:;" class="wt-btn pull-right wt-save-milstone" data-id="<?php echo intval($proposal_id);?>" data-milestone_id="<?php echo intval($post->ID);?>"><?php esc_html_e('Update','workreap');?></a>
															</div>
														</fieldset>
													</form>
												</div>
			...
			<div class="wt-modaldialog modal-dialog" role="document">
				<div class="wt-modalcontent modal-content">
					<div class="wt-boxtitle">
						<h2><?php esc_html_e('Cancelled reason','workreap');?><i class=" wt-btncancel fa fa-times" data-dismiss="modal" aria-label="Close"></i></h2>
					</div>
				...
				<div class="modal-body">
					<form class="wt-formtheme wt-milestone-form">
						<fieldset>
							...
							<div class="form-group form-group-half wt-inputwithicon" data-provide="datepicker">
								<i class="lnr lnr-calendar-full"></i>
								<input id="wt-startdate" type="text" name="due_date" class="form-control <?php echo esc_attr($datepicker_class);?>" placeholder="<?php esc_attr_e('Due Date','workreap');?>">
							</div>
							<div class="form-group form-group-half">
							...
							<div class="form-group">
								<textarea class="form-control" name="description" placeholder="<?php esc_attr_e('Description','workreap');?>"></textarea>
							</div>
							<div class="form-group wt-btnarea">
								<a href="javascript:;" class="wt-btn pull-right wt-save-milstone" data-_milestone_id="" data-id="<?php echo intval($proposal_id);?>"><?php esc_html_e('Save','workreap');?></a>
							</div>
						</fieldset>
					</form>
				</div>
		...
		<div class="wt-modalcontent modal-content">
			...
			<div class="wt-modalbody modal-body">
				<form class="wt-formtheme wt-formfeedback-cancelled wt-formfeedback">
					<fieldset>
						<div class="form-group">
							<textarea class="form-control cancelled-feedback" name="cancelled_reason" placeholder="<?php esc_attr_e('Add Your Feedback','workreap');?>"></textarea>
						</div>
						<div class="form-group wt-btnarea">
							<a class="wt-btn cancelled-btn" data-project-id="<?php echo intval($project_id);?>" href="javascript:;">
			...
			<div class="wt-modalbody modal-body">
				<form class="wt-formtheme wt-formfeedback-complete wt-formfeedback">
					<fieldset>
						...
						<?php } ?>
						<div class="form-group wt-btnarea">
							<a class="wt-btn compelete-btn" data-project-id="<?php echo intval($project_id);?>" href="javascript:;">
								<?php esc_html_e('Send Feedback','workreap');?>
							</a>
						</div>
					</fieldset>
