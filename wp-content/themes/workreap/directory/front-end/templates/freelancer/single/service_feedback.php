...
$service_id		= $post->ID;
$show_posts 	= 3;//get_option('posts_per_page') ? get_option('posts_per_page') : 10;
		...
		<div class="wt-reviews">
			...
			if (!empty($count_post) && $count_post > $show_posts) {?>
				<div class="wt-btnarea">
					<a href="javascript:;" class="wt-btn wt-more-reviews wt-more-rating-service" data-id="<?php echo intval( $post->ID );?>"><?php esc_html_e('Load More','workreap');?></a>
				</div>
			<?php }
		} else{
			do_action('workreap_empty_records_html','wt-empty-person',esc_html__( 'No Service reviews.', 'workreap' ),true);
		}
	?>
</div>
